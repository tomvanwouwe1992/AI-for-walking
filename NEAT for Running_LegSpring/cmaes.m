...
function result = cmaes(population)
global NEAT_PARAMS
Path = pwd;
...
CostFunction=@ForwardIntegrationRunner;   % Cost Function


nVar=size(individual.connectiongenes,2);                % Number of Unknown (Decision) Variables

VarSize=[1 nVar];       % Decision Variables Matrix Size

VarMin=-4;             % Lower Bound of Decision Variables
VarMax= 4;             % Upper Bound of Decision Variables
...
M(1).Cost = CostFunction(M(1).Position,model_name,timevector, individual.nodegenes, individual.connectiongenes,NN_info,print);
...
for g=1:MaxIt
    ...
    parfor i=1:lambda
        ...
        pop(i).Cost=CostFunction(pop(i).Position,model_name,timevector, individual.nodegenes, individual.connectiongenes,NN_info,print);
    ...
    M(g+1).Cost=CostFunction(M(g+1).Position,model_name,timevector, individual.nodegenes, individual.connectiongenes,NN_info,print);
