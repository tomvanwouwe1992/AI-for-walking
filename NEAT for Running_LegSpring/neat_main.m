...
crossover.probability_interspecies=0.001;
...
mutation.probability_add_node=0.002;        % Chance of adding a node
mutation.probability_add_connection=0.002;  % Chance of adding a connection
mutation.probability_recurrency=0.2;        % If we are in add_connection_mutation, this governs if a recurrent connection is allowed. Note: this will only activate if the random connection is a recurrent one, otherwise the connection is simply accepted. If no possible non-recurrent connections exist for the current node genes, then for e.g. a probability of 0.1, 9 times out of 10 no connection is added.
mutation.probability_mutate_weight=0.30;    % Chance of mutating the weights
...
mutation.weight_range=1.5;                  % Random distribution with width mutation.weight_range, centered on 0. mutation range of 5 will give random distribution from -2.5 to 2.5
mutation.probability_gene_reenabled=0.15;   % Probability of a connection gene being reenabled in offspring if it was inherited disabled
